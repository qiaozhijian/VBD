...
class TrackingReward(nn.Module):
    ...
    def forward(self, traj_pred: torch.Tensor, traj_ref: torch.Tensor, weight: torch.Tensor = None, **kwargs):
        """
        ...
        if len(weight.shape) == (len(traj_ref.shape)-1):
            ...
            assert weight.shape[-1] == traj_ref.shape[-1], "weight shape must be either (batch, seq) or same as traj_ref"
        else:
            raise ValueError("weight shape must be either (B, A, T) or same as traj_ref")
        
        rewards = -self.loss_fn(input=traj_pred[..., :d], 
                                target=traj_ref[..., :d], reduction="none") * weight
