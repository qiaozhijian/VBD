...
def distance_offroad(
    ...
    bbox_corners = torch.reshape(
        bbox_corners, [-1, dim]
    )

    # Shape: (...*num_corners=4).
    distances, sign = compute_signed_distance_to_nearest_road_edge_point(bbox_corners, roadgraph_points)
...
def compute_signed_distance_to_nearest_road_edge_point(
    ...
    sampled_points_torch = torch.from_numpy(sampled_points).type_as(query_points)
    ...
    # Shape: (num_map_points).
    prior_point_in_same_curve = id_torch[nearest_indices] == id_torch[prior_indices]
    ...
    offroad_sign = torch.where(offroad_sign == 0, torch.ones_like(offroad_sign), offroad_sign)
    # Shape: (num_query_points).
